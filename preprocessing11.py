...
# ════════════════════════════════════════════════════════════════
    ...
    # ── 1. PETA PLTB EXISTING ──────────────────────────────────
    st.markdown("<div class='section-title' style='font-size:18px;'>Peta Lokasi PLTB Existing di Sulawesi Selatan</div>", unsafe_allow_html=True)

    PLTB_SITES = [
        {"name": "PLTB Sidrap (Sidenreng Rappang)",  "lat": -3.8520, "lon": 119.9860, "kapasitas": "75 MW",  "status": "Beroperasi",  "turbin": "30 × Vestas V110-2.5MW"},
        {"name": "PLTB Tolo (Jeneponto)",              "lat": -5.6740, "lon": 119.7420, "kapasitas": "72 MW",  "status": "Beroperasi",  "turbin": "24 × Vestas V136-3.0MW"},
        {"name": "PLTB Jeneponto II (Rencana)",        "lat": -5.7100, "lon": 119.7800, "kapasitas": "50 MW",  "status": "Perencanaan", "turbin": "—"},
        {"name": "Lokasi Kajian ERA5 (Jeneponto)",     "lat": -6.4000, "lon": 119.0000, "kapasitas": "—",      "status": "Studi",       "turbin": "Area analisis dashboard ini"},
    ]

    # Buat peta Plotly scatter mapbox
    import json
...
<script>
var sites = """ + json.dumps(PLTB_SITES) + """;

var colors = sites.map(s => s.status === "Beroperasi" ? "#00e5ff" : s.status === "Perencanaan" ? "#ffb300" : "#a0aec0");
var sizes  = sites.map(s => s.status === "Beroperasi" ? 18 : s.status === "Perencanaan" ? 14 : 12);
var symbols = sites.map(s => s.status === "Studi" ? "circle-open" : "circle");
...
var layout = {
    mapbox: {
        ...
        center: {lat: -5.2, lon: 119.7},
        zoom: 7,
    },
    margin: {t:0, b:0, l:0, r:0},
    paper_bgcolor: "#0d1235",
    font: {color: "#ffffff"},
    legend: {bgcolor: "rgba(13,18,53,0.8)", bordercolor: "#2a3a8c", borderwidth: 1},
};
...
"""
    ...
    with col1:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    with col2:
        for site in PLTB_SITES:
            color = "#63D9E8" if site["status"] == "Beroperasi" else "#ffb300" if site["status"] == "Perencanaan" else "#a0aec0"
            st.markdown(f"""
            <div class='stat-box' style='margin-bottom:10px;'>
                <h3 style='color:{color};font-size:11px;'>{site['name']}</h3>
                <p><span class='stat-label'>Status</span><span class='stat-val' style='color:{color};'>{site['status']}</span></p>
    ...
    with col1:
        st.markdown("<div class='plot-card'>", unsafe_allow_html=True)
        load_spasial_plot("weibull", musim_pltb, height=540)
        st.markdown("</div>", unsafe_allow_html=True)
    with col2:
    ...
    with col_result:
        # Ambil mean speed dari musim aktif
    ...
    <p class='about-txt' style='margin-bottom:14px;'>
        ...
        notebook Colab — untuk menghitung estimasi mean speed, power density, dan
        energi tahunan pada kondisi lokasi yang berbeda.
    </p>
    """, unsafe_allow_html=True)

    col_k, col_c, col_d2 = st.columns(3)
    with col_k:
    ...
    with col_d2:
        D_calc = st.number_input("Diameter rotor (m) ", min_value=1.0, max_value=200.0, value=110.0, step=1.0)

    col_cp, col_h = st.columns(2)
    with col_cp:
        Cp_calc = st.slider("Koefisien daya Cp ", min_value=0.10, max_value=0.593, value=0.45, step=0.01)
    with col_h:
        h_factor = st.number_input(
            "Faktor koreksi ketinggian (hub height / 10m)",
            min_value=0.5, max_value=5.0, value=1.0, step=0.05,
            help="Gunakan hukum pangkat: (H_hub/H_ref)^α, α≈0.14. Contoh hub 80m: (80/10)^0.14 ≈ 1.28"
        )

    # Hitung dengan rumus Weibull closed-form
    try:
